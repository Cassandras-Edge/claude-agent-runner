...
import { FIRST_EVENT_TIMEOUT_MS, FORK_FROM, FORK_AT, FORK_SESSION, PATCHED_CLI_PATH, ADDITIONAL_DIRECTORIES } from "./config.js";
import { drainBackground } from "./background-drainer.js";
import { buildClaudeChildEnv, buildZeroEventError, createStderrRingBuffer } from "./helpers.js";
...
export async function runAgentV1(
  ...
  const maxTurns = overrides?.maxTurns ?? state.MAX_TURNS;
  const requestId = overrides?.requestId;
  const traceId = overrides?.traceId;
  ...
  const effectiveCompactInstructions = overrides?.compactInstructionsOverride ?? state.activeCompactInstructions;
  ...
  const response = query({
    ...
    options: {
      ...
      ...(state.ALLOWED_TOOLS.length > 0 ? { tools: state.ALLOWED_TOOLS } : {}),
      ...(state.DISALLOWED_TOOLS.length > 0 ? { disallowedTools: state.DISALLOWED_TOOLS } : {}),
      cwd: state.WORKSPACE,
      includePartialMessages: true,
      persistSession: true,
      enableFileCheckpointing: true,
      ...((state.sdkSessionId || FORK_FROM) ? { resume: state.sdkSessionId || FORK_FROM } : {}),
      ...(FORK_SESSION && !state.sdkSessionId ? { forkSession: true } : {}),
      ...(FORK_AT && !state.sdkSessionId ? { resumeSessionAt: FORK_AT } : {}),
      ...
      stderr: (data: string) => {
        ...
        logger.debug("runner.agent", "sdk_stderr", { session_id: state.SESSION_ID, data: data.trim().slice(0, 500) });
      },
    },
  });
  ...
  try {
    try {
      for await (const event of response) {
        ...
        if (!state.sdkSessionId && "session_id" in event && event.session_id) {
          state.sdkSessionId = event.session_id;
          if (ws.readyState === WebSocket.OPEN) {
            ws.send(JSON.stringify({
              type: "session_init",
              session_id: state.SESSION_ID,
