...
function spawnInteractiveInTmux(config: SpawnConfig, sdkSocketPath: string): ChildProcess {
  ...
  // Claude Code starts in $HOME by default. Use cd + exec to start in workspace.
  const innerCmd = [executable, ...execArgs].map(a => a.includes(" ") ? `"${a}"` : a).join(" ");
  const claudeCmd = `cd ${cwd}; exec ${innerCmd}`;
